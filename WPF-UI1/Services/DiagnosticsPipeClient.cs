...
namespace WPF_UI1.Services
{
    ...
    {
        public int tempSensors { get; set; }
        public int tempSensorCount { get; set; }
        public int smartDisks { get; set; }
        public int smartDiskCount { get; set; }
        public int futureReserved { get; set; }
        public int sharedmemHash { get; set; }
        public int usbDevices { get; set; }
        public int usbDeviceCount { get; set; }
        public int extensionPad { get; set; }
    }
    ...
    public sealed class DiagnosticsPipeClient : IDisposable
    {
        ...
        private async Task RunAsync()
        {
            while (!_cts.IsCancellationRequested)
            {
                try
                {
                    using var pipe = new NamedPipeClientStream(".", "SysMonDiag", PipeDirection.In);
                    await pipe.ConnectAsync(3000, _cts.Token);
                    using var br = new BinaryReader(pipe, Encoding.UTF8, false);
                    while (pipe.IsConnected && !_cts.IsCancellationRequested)
                    {
                        ...
                        try
                        {
                            var snap = JsonSerializer.Deserialize<DiagnosticsPipeSnapshot>(json);
                            if (snap != null) SnapshotReceived?.Invoke(snap);
                        }
                        catch (Exception ex)
                        {
                            Log.Debug("解析JSON失败: " + ex.Message);
                        }
                    }
                }
                catch (Exception ex)
                {
                    Log.Debug("管道连接失败: " + ex.Message);
