...
// 将 UTF-8 字面量转换为 std::string，避免中文字符串导致的编译/编码问题
static inline std::string Utf8(const char8_t* s) {
...
static void LogRealGpuNames(const std::vector<GpuInfo::GpuData>& gpus, bool isDetailedLogging) {
    ...
    if (!realGpuNames.empty()) {
        std::string msg = Utf8(u8"TemperatureWrapper: 真实GPU名称列表: ");
        ...
        Logger::Debug(Utf8(u8"TemperatureWrapper: 未检测到真实GPU"));
    }
}
...
void TemperatureWrapper::Initialize() {
    try {
        ...
        if (!gpuInfo && wmiManager && wmiManager->IsInitialized()) {
            ...
            Logger::Warn(Utf8(u8"TemperatureWrapper: WmiManager初始化失败，无法获取本地GPU温度"));
        }
    }
    catch (...) {
        initialized = false;
...
std::vector<std::pair<std::string, double>> TemperatureWrapper::GetTemperatures() {
    ...
    if (initialized) {
        try {
            ...
            if (isDetailedLogging) {
                Logger::Debug(Utf8(u8"TemperatureWrapper: 从libre获取温度传感器数量: ") + std::to_string(libreTemps.size()));
            }
            temps.insert(temps.end(), libreTemps.begin(), libreTemps.end());
        }
        catch (...) {
            if (isDetailedLogging) {
                Logger::Warn(Utf8(u8"TemperatureWrapper: 获取libre温度异常"));
            }
        }
    }
    ...
    if (gpuInfo) {
        ...
        if (isDetailedLogging) {
            Logger::Debug(Utf8(u8"TemperatureWrapper: GpuInfo GPU数量: ") + std::to_string(gpus.size()));
        ...
        for (const auto& gpu : gpus) {
            if (gpu.isVirtual) {
                if (isDetailedLogging) {
                    Logger::Debug(Utf8(u8"TemperatureWrapper: 跳过虚拟GPU: ") + std::string(gpu.name.begin(), gpu.name.end()));
                }
                continue;
            }
            std::string gpuName(gpu.name.begin(), gpu.name.end());
            if (isDetailedLogging) {
                Logger::Debug(Utf8(u8"TemperatureWrapper: GpuInfo检测到GPU: ") + gpuName + Utf8(u8", 温度: ") + std::to_string(gpu.temperature));
            }
        ...
        if (isDetailedLogging) {
            Logger::Warn(Utf8(u8"TemperatureWrapper: GpuInfo未初始化"));
        }
    }

    if (isDetailedLogging) {
        Logger::Debug(Utf8(u8"TemperatureWrapper: 总温度数量: ") + std::to_string(temps.size()));
    }
